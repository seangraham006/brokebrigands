...
fight_bandits() {
    ...
    for i in $(seq 1 $num_enemies); do
        health=$((RANDOM % 50 + 50))
        damage=$((RANDOM % 5 + 5))
        name="Enemy$i"
        enemy_healths+=("$health")
        enemy_damages+=("$damage")
        enemy_names+=("$name")
    done

    echo "Enemy Group Details:"
    for i in "${!enemy_names[@]}"; do
        echo "Name: ${enemy_names[i]}, Health: ${enemy_healths[i]}, Damage: ${enemy_damages[i]}"
    done
